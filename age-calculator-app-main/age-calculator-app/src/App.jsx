...
function App() {
  ...
  function validateEntry() {
    ...
    if (year === "" || yearInt > 2023 || yearInt < 1900) { setYearError(true); setAge(reset) } else { setYearError(false) }

    const calcDay = (parseInt(currentDay, 10) - parseInt(day, 10))
    const calcMonth = (parseInt(currentMonth, 10) - parseInt(month, 10));

    if ((calcDay < 0 || calcMonth < 0) && currentYear === 2023) {
      setDayError(true);
      setMonthError(true);
      setYearError(true)
  ...
  return (
    <>
      ...
      <div>
        <div>
          <h1>
            {age.year === "" && age.year !== 0 ? <p>--</p> : <p>{age.year}</p>}years</h1>
        </div>
        <div>
          <h1>{age.month === "" && age.month !== 0 ? <p>--</p> : <p>{age.month}</p>}months</h1>
        </div>
        <div>
          <h1>{age.day === "" && age.day !== 0 ? <p>--</p> : <p>{age.day}</p>}days</h1>
        </div>
      </div>
    </>
  )
